trigger UpdateTeamStats on Match__c (before insert, before update) {
    ...
    for (Match__c match : Trigger.new) {
        if (String.isNotBlank(match.Result__c) && match.Result__c.contains('-') && match.Tournament_Group__c != null) {
